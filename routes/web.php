...
Route::get('/dearSanta/{participant}', [DearSantaController::class, 'view'])->name('dearSanta')
    ->missing(function () {
...
Route::middleware(['signed', 'decrypt.iv:participant,name'])->group(function () {
    ...
    Route::post('/participant/{participant}/sub', function(Participant $participant, Request $request) {
        $participant->updatePushSubscription($request->input('endpoint'), $request->input('keys.p256dh'), $request->input('keys.auth'));
...
Route::get('/participant/{participant}/acceptRedraw', [RedrawController::class, 'accept'])->name('acceptRedraw');
...
Route::middleware(['signed', 'decrypt.iv:draw,mail_title'])->group(function () {
    ...
    Route::get('/org/{draw}/suggestRedraw', [RedrawController::class, 'suggestRedraw'])->name('organizerPanel.suggestRedraw');
    Route::get('/org/{draw}/redraw', [RedrawController::class, 'redraw'])->name('organizerPanel.redraw');
});
Route::middleware(['signed', 'decrypt.iv:participant,name'])->group(function () {
