    ...
    <tr class="participant" :id="'participant_'+idx">
        <td class="align-middle">
            <div class="input-group">
                ...
                <input type="text" :name="'participants['+idx+'][name]'" :required="idx < 3" placeholder="@lang('form.name.placeholder')" :value="name" @input="$emit('input:name', $event.target.value)" class="form-control participant-name" />
            </div>
        </td>
        <td class="border-left align-middle">
            <input type="email" :name="'participants['+idx+'][email]'" placeholder="@lang('form.email.placeholder')" :value="email" @input="$emit('input:email', $event.target.value)" class="form-control participant-email" :required="(idx < 3 || name !== '')" />
        </td>
        <td class="border-right text-left participant-exclusions-wrapper align-middle">
            <multiselect :options="participantNames" :value="exclusions" @input="$emit('input:exclusions', $event)" track-by="value" label="text" placeholder="@lang('form.exclusions.placeholder')" :multiple="true"></multiselect>
